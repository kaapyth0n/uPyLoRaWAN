...
class ErrorLogger:
	...
	def log_error(self, error_type, message, severity=2):
		error_entry = {'timestamp': time.time(), 'type': error_type, 'message': message[:100], 'severity': severity}
		self.errors.append(error_entry)
		while len(self.errors) > self.max_entries:
			self.errors.pop(0)
		try:
			if hasattr(self.controller, 'mqtt_handler') and self.controller.mqtt_handler.initialized:
				self.controller.mqtt_handler.publish_error(error_type, message, severity)
		except:
			pass
	...
	def get_error_stats(self):
		stats = {'total': len(self.errors), 'by_severity': {self.INFO: 0, self.WARNING: 0, self.ERROR: 0, self.CRITICAL: 0}, 'by_type': {}}
