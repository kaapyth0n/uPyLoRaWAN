...
class MQTTClient:
	...
	def connect(self, clean_session=True, timeout=None):
		...
		msg = bytearray(b'\x04MQTT\x04\x02\x00\x00')
		...
		if self.keepalive:
			...
			msg[8] |= self.keepalive & 255
		if self.lw_topic:
			sz += 2 + len(self.lw_topic) + 2 + len(self.lw_msg)
			msg[6] |= 4 | (self.lw_qos & 1) << 3 | (self.lw_qos & 2) << 3
