...
class SmartBoilerInterface(ObjectInterface, BoilerInterface):
    ...
    def _test_ssr_module(self):
        try:
            if self.fr.read(0, slot=5) is None:
                return False
            self.fr.write(6, 1, slot=5)
            time.sleep(0.1)
            self.fr.write(6, 0, slot=5)
            return True
        except Exception as e:
        ...
        while True:
            try:
                ...
                if current_state == SystemState.ERROR:
                    self.display_manager.show_status('Error State', 'Recovery attempt', 'in progress...')
                    time.sleep(5)
                elif current_state == SystemState.SAFE_MODE:
                    self.display_manager.show_status('Safe Mode', 'Manual reset', 'required')
                    time.sleep(30)
    ...
    def _process_notifications(self):
        try:
            ...
            self.logger.log_error('notification', f'Error processing parameter change notification: {e}', severity=2)
        ...
        try:
            ...
            if param_name in pid_params:
                if param_name == 'mode':
                    if value != 'pid':
                        self._pid_configured = False
                elif self._get_mode() == 'pid':
                    self._pid_configured = False
                    self.logger.log_error('control', f'PID parameter {param_name} changed - will reconfigure', severity=1)
        except Exception as e:
            self.logger.log_error('control', f'Config change handler error: {e}', severity=2)
    ...
    def _update_control_logic(self):
        try:
            ...
            if mode == 'pid':
                ...
                voltage = self.fr.read(24, slot=6)
                if voltage is None:
                    voltage = 0.0
                self.heating_active = voltage > 1.0
            elif mode == 'soft_pid':
                if self._pid_configured:
                    try:
                        self.fr.write(26, 0, slot=6)
                        self._pid_configured = False
                        self.logger.log_error('control', 'Hardware PID controller disabled when switching to soft_pid mode', severity=1)
                    except Exception as e:
                ...
                if dt >= pid_dt:
                    output_voltage = self.temp_controller.calculate_soft_pid_output(self.current_temp, self._get_setpoint(), dt)
    ...
    def _configure_pid(self):
        try:
            ...
            self.logger.log_error('control', 'PID controller configured successfully', severity=1)
            return True
        except Exception as e:
            ...
            try:
                ...
                if wifi_config:
                    ...
                    if not sta_if.isconnected():
                        self.logger.log_error('wifi', 'WiFi connection lost - attempting reconnection', severity=1)
                        utils.force_reconnect(sta_if, wifi_config['ssid'], wifi_config['password'])
            except Exception as e:
    ...
    def _update_display_status(self):
        try:
            ...
            if mode == 'pid':
                try:
                    output_voltage = self.fr.read(24, slot=6)
                except:
                    pass
            devaddr = None
            ...
            self.logger.log_error('display', f'Display status update error: {e}', severity=2)
            return False
        ...
        while retry_count < retries:
            try:
                ...
                if retry_count == retries:
                    self.logger.log_error('control', f'Failed to verify relay state: {e}', severity=2)
        return False
            ...
            if not self.heating_active:
                ...
                if current_time - self.last_state_change >= self.config_manager.get_param('min_off_time'):
                    ...
                    if self._verify_relay_state(1):
                        ...
                        self.logger.log_error('control', 'Failed to activate heating - state verification failed', severity=3)
        except Exception as e:
            ...
            if self.heating_active:
                ...
                if current_time - self.last_state_change >= self.config_manager.get_param('min_on_time'):
                    ...
                    if self._verify_relay_state(0):
                        ...
                        self.logger.log_error('control', 'Failed to deactivate heating - state verification failed', severity=3)
        except Exception as e:
    ...
    def _safe_shutdown(self):
        try:
            ...
            if self.display_manager:
                self.display_manager.show_status('System Shutdown', 'Safe mode', 'Restarting...')
            self.logger.log_error('system', 'Safe shutdown initiated', severity=3)
        except Exception as e:
            pass
        finally:
            time.sleep(1)
