...
REG_PA_CONFIG = 9
REG_LNA = 12
REG_FIFO_ADDR_PTR = 13
REG_FIFO_TX_BASE_ADDR = 14
FifoRxBaseAddr = 0
FifoTxBaseAddr = 0
REG_FIFO_RX_BASE_ADDR = 15
FifoRxBaseAddr = 0
REG_FIFO_RX_CURRENT_ADDR = 16
REG_IRQ_FLAGS_MASK = 17
REG_IRQ_FLAGS = 18
REG_RX_NB_BYTES = 19
REG_PKT_RSSI_VALUE = 26
REG_PKT_SNR_VALUE = 27
REG_FEI_MSB = 29
REG_FEI_LSB = 30
REG_MODEM_CONFIG = 38
REG_PREAMBLE_DETECT = 31
REG_PREAMBLE_MSB = 32
REG_PREAMBLE_LSB = 33
REG_PAYLOAD_LENGTH = 34
REG_FIFO_RX_BYTE_ADDR = 37
REG_RSSI_WIDEBAND = 44
REG_DETECTION_OPTIMIZE = 49
REG_DETECTION_THRESHOLD = 55
REG_SYNC_WORD = 57
REG_DIO_MAPPING_1 = 64
REG_VERSION = 66
REG_INVERTIQ = 51
RFLR_INVERTIQ_RX_MASK = 191
RFLR_INVERTIQ_RX_OFF = 0
RFLR_INVERTIQ_RX_ON = 64
RFLR_INVERTIQ_TX_MASK = 254
RFLR_INVERTIQ_TX_OFF = 1
RFLR_INVERTIQ_TX_ON = 0
REG_INVERTIQ2 = 59
RFLR_INVERTIQ2_ON = 25
RFLR_INVERTIQ2_OFF = 29
MODE_LONG_RANGE_MODE = 128
MODE_SLEEP = 0
MODE_STDBY = 1
MODE_TX = 3
MODE_RX_CONTINUOUS = 5
MODE_RX_SINGLE = 6
PA_BOOST = 128
IRQ_TX_DONE_MASK = 8
IRQ_PAYLOAD_CRC_ERROR_MASK = 32
IRQ_RX_DONE_MASK = 64
IRQ_RX_TIME_OUT_MASK = 128
MAX_PKT_LENGTH = 255
__DEBUG__ = False
...
class SX127x:
    _default_parameters = {'tx_power_level': 2, 'signal_bandwidth': 'SF7BW125', 'spreading_factor': 7, 'coding_rate': 5, 'sync_word': 52, 'implicit_header': False, 'preamble_length': 8, 'enable_CRC': False, 'invert_IQ': False}
    _data_rates = {'SF7BW125': (116, 114, 4), 'SF7BW250': (116, 130, 4), 'SF8BW125': (132, 114, 4), 'SF9BW125': (148, 114, 4), 'SF10BW125': (164, 114, 4), 'SF11BW125': (180, 114, 12), 'SF12BW125': (196, 114, 12)}

    def __init__(self, spi, pins, ttn_config, channel=0, fport=1, lora_parameters=_default_parameters):
        self._spi = spi
        self._pins = pins
        ...
        self.write_register(REG_LNA, self.read_register(REG_LNA) | 3)
        self.write_register(REG_MODEM_CONFIG, 4)
    ...
    def end_packet(self, timeout=5):
        ...
        while not timed_out and irq_value & IRQ_TX_DONE_MASK == 0:
    ...
    def send_data(self, data, data_length, frame_counter, timeout=5):
        ...
        aes = AES(self._ttn_config.device_address, self._ttn_config.app_key, self._ttn_config.network_key, self.frame_counter)
        ...
        lora_pkt[5] = 0
        lora_pkt[6] = self.frame_counter & 255
        lora_pkt[7] = self.frame_counter >> 8 & 255
    ...
    def get_irq_flags(self):
        ...
        if __DEBUG__:
            irq_dict = dict(rx_timeout=irq_flags >> 7 & 1, rx_done=irq_flags >> 6 & 1, crc_error=irq_flags >> 5 & 1, valid_header=irq_flags >> 4 & 1, tx_done=irq_flags >> 3 & 1, cad_done=irq_flags >> 2 & 1, fhss_change_ch=irq_flags >> 1 & 1, cad_detected=irq_flags >> 0 & 1)
    ...
    def set_tx_power(self, level, outputPin=PA_OUTPUT_PA_BOOST_PIN):
        ...
        if outputPin == PA_OUTPUT_RFO_PIN:
            ...
            self.write_register(REG_PA_CONFIG, 112 | level)
        else:
    ...
    def set_coding_rate(self, denominator):
        ...
        cr = denominator - 4
        self.write_register(REG_FEI_MSB, self.read_register(REG_FEI_MSB) & 241 | cr << 1)

    def set_preamble_length(self, length):
        self.write_register(REG_PREAMBLE_MSB, length >> 8 & 255)
        self.write_register(REG_PREAMBLE_LSB, length >> 0 & 255)

    def set_spreading_factor(self, sf):
        sf = min(max(sf, 6), 12)
        self.write_register(REG_DETECTION_OPTIMIZE, 197 if sf == 6 else 195)
        self.write_register(REG_DETECTION_THRESHOLD, 12 if sf == 6 else 10)
        self.write_register(REG_FEI_LSB, self.read_register(REG_FEI_LSB) & 15 | sf << 4 & 240)
    ...
    def enable_CRC(self, enable_CRC=False):
        ...
        config = modem_config_2 | 4 if enable_CRC else modem_config_2 & 251
        self.write_register(REG_FEI_LSB, config)

    def invert_IQ(self, invert_IQ):
        self._parameters['invertIQ'] = invert_IQ
        if invert_IQ:
            self.write_register(REG_INVERTIQ, self.read_register(REG_INVERTIQ) & RFLR_INVERTIQ_TX_MASK & RFLR_INVERTIQ_RX_MASK | RFLR_INVERTIQ_RX_ON | RFLR_INVERTIQ_TX_ON)
            self.write_register(REG_INVERTIQ2, RFLR_INVERTIQ2_ON)
        else:
            self.write_register(REG_INVERTIQ, self.read_register(REG_INVERTIQ) & RFLR_INVERTIQ_TX_MASK & RFLR_INVERTIQ_RX_MASK | RFLR_INVERTIQ_RX_OFF | RFLR_INVERTIQ_TX_OFF)
    ...
    def implicit_header_mode(self, implicit_header_mode=False):
        ...
        config = modem_config_1 | 1 if implicit_header_mode else modem_config_1 & 254
        self.write_register(REG_FEI_MSB, config)

    def receive(self, size=0):
        self.implicit_header_mode(size > 0)
        if size > 0:
            self.write_register(REG_PAYLOAD_LENGTH, size & 255)
    ...
    def on_receive(self, callback):
        ...
        if self._pin_rx_done:
            if callback:
                self.write_register(REG_DIO_MAPPING_1, 0)
                self._pin_rx_done.irq(trigger=Pin.IRQ_RISING, handler=self.handle_on_receive)
            else:
    ...
    def handle_on_receive(self, event_source):
        ...
        if self._on_receive:
            ...
            aes = AES(self._ttn_config.device_address, self._ttn_config.app_key, self._ttn_config.network_key, frame_counter)
    ...
    def read_register(self, address, byteorder='big', signed=False):
        response = self.transfer(address & 127)
        return int.from_bytes(response, byteorder)

    def write_register(self, address, value):
        self.transfer(address | 128, value)

    def transfer(self, address, value=0):
