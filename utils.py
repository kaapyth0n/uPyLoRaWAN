...
def memory_stats():
	...
	alloc = gc.mem_alloc()
	total = free + alloc
	return {'free': free, 'allocated': alloc, 'total': total, 'percent_used': alloc * 100 / total}
...
def calc_temperature_stats(temp_history):
	if not temp_history:
		return {'min': None, 'max': None, 'avg': None, 'trend': 0}
	try:
		temps = [t[1] for t in temp_history]
		return {'min': min(temps), 'max': max(temps), 'avg': sum(temps) / len(temps), 'trend': calc_trend(temp_history)}
	except:
		return {'min': None, 'max': None, 'avg': None, 'trend': 0}
...
def calc_trend(history, window=300):
	try:
		...
		filtered = [(t, v) for t, v in history if current_time - t <= window]
		if len(filtered) < 2:
			return 0
